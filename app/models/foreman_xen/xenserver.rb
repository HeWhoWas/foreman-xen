module ForemanXen
  class Xenserver < ComputeResource
    ...
    def create_vm_from_builtin(args)
      ...
      volumes = args[:volumes_attributes]
      nics = args[:volumes_attributes]
      ...
      #Create the volumes on the selected storage repositories.
      vdis = []
      volumes.each do | name, details |
        storage_repository = client.storage_repositories.find { |sr| sr.uuid == details[:sr_uuid] }
        vol_size = details[:size_gb].to_i * gb_bytes
        vdi = client.vdis.create  :name               => "#{args[:name]}-disk#{i}",
                                  :storage_repository => storage_repository,
                                  :description        => "#{args[:name]}-disk_#{i}",
                                  :virtual_size       => vol_size.to_s
        vdis << vdi
        i=i+1;
      end
      ...
      i = 0
      nics.each do | index, nic |
        create_network(vm, nic[:print], i)
        vm.reload
        i=i+1;
      end
    ...
    def create_network(vm, network_name, dev=0)
      net        = client.networks.find { |n| n.name == network_name }
      net_config = {
